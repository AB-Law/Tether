...
RELATIONSHIP_TYPES = ("close_friend", "friend", "acquaintance", "new_person", "complicated", "faded")
...
def upgrade() -> None:
    ...
    op.create_index("ix_people_user_archived_name", "people", ["user_id", "archived_at", "name"], unique=False)
    op.create_index("ix_people_user_next_nudge", "people", ["user_id", "next_nudge_at"], unique=False)
    op.create_index("ix_people_user_last_talked", "people", ["user_id", "last_talked_at"], unique=False)
    op.create_index(
        "ix_people_name_trgm",
        "people",
        ["name"],
