...
from app.services import auth_service, moment_service, people_service, reminder_service, warmth_service
...
async def test_services_cover_happy_and_error_paths(monkeypatch):
    ...
    with pytest.raises(exceptions.AppException):
        await auth_service.get_current_user("t", db4)  # type: ignore[arg-type]
    monkeypatch.setattr(
        auth_service.user_repository, "get_by_id", AsyncMock(return_value=SimpleNamespace(is_active=False))
    )
    with pytest.raises(exceptions.AppException):
        await auth_service.get_current_user("t", db4)  # type: ignore[arg-type]
    active_user = SimpleNamespace(is_active=True)
    monkeypatch.setattr(auth_service.user_repository, "get_by_id", AsyncMock(return_value=active_user))
    ...
    monkeypatch.setattr(moment_service.person_repository, "get_by_id", AsyncMock(return_value=SimpleNamespace()))
    monkeypatch.setattr(moment_service.moment_repository, "list_by_person", AsyncMock(return_value=[1]))
    ...
    monkeypatch.setattr(moment_service.moment_repository, "get_by_id", AsyncMock(return_value=moment_obj))
    ...
    monkeypatch.setattr(moment_service.person_repository, "get_by_id", AsyncMock(return_value=person_obj))
    monkeypatch.setattr(moment_service.moment_repository, "create", AsyncMock(return_value=moment_obj))
    monkeypatch.setattr(
        moment_service,
        "warmth_service",
    ...
    with pytest.raises(exceptions.AppException):
        await moment_service.create_moment({"person_id": pid}, uid, db4)  # type: ignore[arg-type]
    monkeypatch.setattr(moment_service.moment_repository, "update", AsyncMock(return_value=moment_obj))
    await moment_service.update_moment(mid, uid, {"x": 1}, db4)  # type: ignore[arg-type]
    monkeypatch.setattr(moment_service.moment_repository, "soft_delete", AsyncMock(return_value=moment_obj))
    await moment_service.delete_moment(mid, uid, db4)  # type: ignore[arg-type]
    monkeypatch.setattr(people_service.person_repository, "list_people", AsyncMock(return_value=(["p"], 7)))
    ...
    monkeypatch.setattr(people_service.person_repository, "get_by_id", AsyncMock(return_value=SimpleNamespace(id=pid)))
    person_created = SimpleNamespace(id=pid, contact_cadence_days=3)
    monkeypatch.setattr(people_service.person_repository, "create", AsyncMock(return_value=person_created))
    ...
    monkeypatch.setattr(people_service.person_repository, "update", AsyncMock(return_value=person_created))
    monkeypatch.setattr(people_service.person_repository, "soft_archive", AsyncMock(return_value=person_created))
    ...
    monkeypatch.setattr(people_service.person_repository, "drifting_away", AsyncMock(return_value=[]))
    ...
    monkeypatch.setattr(reminder_service.reminder_repository, "cancel_pending_for_entity", AsyncMock())
    ...
    monkeypatch.setattr(warmth_service.person_repository, "get_by_id", AsyncMock(return_value=target_person))
    monkeypatch.setattr(
        warmth_service.moment_repository,
        "list_active_for_person",
        AsyncMock(return_value=[SimpleNamespace(sentiment=SentimentEnum.WARM.value, occurred_on=date.today())]),
    )
    ...
    monkeypatch.setattr(auth_service.user_repository, "get_by_email", AsyncMock(return_value=inactive))
    with pytest.raises(exceptions.AppException):
        await auth_service.login("a@b.com", "pw", db)  # type: ignore[arg-type]
