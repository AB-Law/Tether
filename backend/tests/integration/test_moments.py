...
async def test_moments_requires_auth(api_client: AsyncClient) -> None:
    response = await api_client.get(
        "/api/v1/moments/00000000-0000-0000-0000-000000000000"
    )
    assert response.status_code == 401


@pytest.mark.asyncio
async def test_moments_crud(
    ...
    fetched = await api_client.get(
        f"/api/v1/moments/{moment_id}", headers=auth_headers
    )
...
async def test_moment_cross_user_access_blocked(api_client: AsyncClient, create_user) -> None:
    owner_headers = await get_auth_headers(
        api_client,
        create_user,
        "moment-owner@example.com",
    )
    other_headers = await get_auth_headers(
        api_client,
        create_user,
        "moment-other@example.com",
    )
