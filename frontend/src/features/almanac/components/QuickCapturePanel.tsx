...
export function QuickCapturePanel() {
  ...
  return (
    <>
      <button aria-label="Close capture panel overlay" className="fixed inset-0 z-40 bg-slate-900/30" onClick={close} type="button" />
      <aside className="fixed right-0 top-0 z-50 h-screen w-full max-w-md border-l border-slate-200 bg-white p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-3">
          ...
          <button className="rounded-lg border border-slate-300 px-2 py-1 text-sm text-slate-600" onClick={close} type="button">
            Close
          </button>
        </div>
        <form
        ...
        >
          ...
          <label className="block space-y-1">
            ...
            <select className="w-full rounded-lg border border-slate-300 px-3 py-2" onChange={(event) => setEntryType(event.target.value as EntryType)} value={entryType}>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            <button className="rounded-lg border border-slate-300 px-3 py-2 text-sm font-medium text-slate-700" onClick={close} type="button">
              Cancel
            </button>
            <button className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700" disabled={captureMutation.isPending} type="submit">
              Capture
            </button>
          </div>
          {errorMessage ? (
      ...
      {showToast ? <div className="fixed bottom-6 right-6 z-[60] rounded-lg bg-slate-900 px-3 py-2 text-sm text-white shadow-lg">Captured successfully</div> : null}
    </>
  )
}
